...
$connexion;
	...
	function recherche($recherche, $page){
		global $connexion; 
		$page = $page * 10;
        $maRequete = $connexion->prepare('SELECT * FROM Utilisateur WHERE Username LIKE :recherche OR nom LIKE :recherche OR prenom LIKE :recherche Order by Username asc LIMIT 10 OFFSET :page');
	...
	function rechercheByRole($recherche, $role, $page){
		global $connexion; 
		$page = $page * 10;
        $maRequete = $connexion->prepare('SELECT * FROM Utilisateur WHERE (Username LIKE :recherche OR nom LIKE :recherche OR prenom LIKE :recherche) AND id_role =:role Order by Username LIMIT 10 OFFSET :page');
		$maRequete->bindValue(':recherche',  '%'.$recherche . '%', PDO::PARAM_STR);
    ...
    function displayAllUsers($page){
		global $connexion;
		$page = $page * 6;
		$maRequete = $connexion->prepare("SELECT * FROM Utilisateur Order by Username LIMIT 6 OFFSET :page;");
