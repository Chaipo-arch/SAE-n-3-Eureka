<?php
  ...
  && isset($_POST['mdp']) && isset($_POST['role']) ){
    ...
    $nomValide = preg_match('/^[a-zA-Z]+$/',$nom) && !preg_match('/\s/',$nom);
    $prenomValide = preg_match('/^[a-zA-Z]+$/',$prenom)&& !preg_match('/\s/',$nom);
    ...
    <div id="container">
      ...
      <form class="formAjoutEntreprise" action="ajoutUtilisateur.php" method="post">
        ...
        <fieldset>
          ...
          <div>
            <label for="first-name">Nom de L'etudiant*  <?php if(isset($nom) && !$nomValide){ echo '<div class="messErreur">le nom ne doit contenir aucune nombre ou caractére spéciaux</div>';}?></label></label>
            <input type="text" name="nomEtudiant" <?php if(isset($nom)){echo 'value="'.$nom.'"';} if(isset($nom) && (!$nomValide)){echo 'class="erreur"';} ?> required >
            <br/>
            <label for="first-name">Prenom de L'etudiant* <?php if(isset($prenom) && !$prenomValide){ echo '<div class="messErreur">le prenom ne doit contenir aucun nombre ou caractére spéciaux </div>';}?></label>
            ...
            <select type="text"class="form-control" id="filiere2" name="filiere" hidden>
